...
const App: React.FC = () => {
  ...
  // ============================================================
  // 顶层页面路由 — 使用 URL hash 持久化页面状态
  // ============================================================
  // 从 hash 解析初始状态：#home, #workspace/13, #anchor-admin, etc.
  const parseHash = useCallback((): { page: AppPage; anchorId?: number } => {
    const hash = window.location.hash.replace('#', '');
    if (!hash || !localStorage.getItem('qc_logged_in')) return { page: localStorage.getItem('qc_logged_in') === '1' ? 'home' : 'login' };
    const parts = hash.split('/');
    const page = parts[0] as AppPage;
    const validPages: AppPage[] = ['home', 'workspace', 'anchor-admin', 'script-admin', 'model-admin', 'training-admin', 'login'];
    if (!validPages.includes(page)) return { page: 'home' };
    if (page === 'workspace' && parts[1]) return { page, anchorId: parseInt(parts[1]) };
    return { page };
  }, []);

  const initState = parseHash();
  const [appPage, setAppPage] = useState<AppPage>(initState.page);
  const [selectedAnchor, setSelectedAnchor] = useState<Anchor | null>(null);
  const [pendingAnchorId, setPendingAnchorId] = useState<number | undefined>(initState.anchorId);

  // 导航函数：同时更新 state 和 hash
  const navigateTo = useCallback((page: AppPage, anchor?: Anchor | null) => {
    setAppPage(page);
    if (anchor) {
      setSelectedAnchor(anchor);
      window.location.hash = `${page}/${anchor.id}`;
    } else {
      if (page !== 'workspace') setSelectedAnchor(null);
  ...
  useEffect(() => {
    const onHashChange = () => {
      const { page, anchorId } = parseHash();
      setAppPage(page);
      if (page === 'workspace' && anchorId) {
  ...
  // 初始化 hash（首次加载如果没有 hash，设一个）
  useEffect(() => {
    if (!window.location.hash && appPage !== 'login') {
      window.location.hash = appPage;
    }
  }, []);

  // 如果有 pendingAnchorId（从 hash 恢复的），异步加载主播数据
  useEffect(() => {
    if (pendingAnchorId && !selectedAnchor) {
      fetch('/api/anchors')
        .then(r => r.json())
        .then(list => {
          const found = list.find((a: Anchor) => a.id === pendingAnchorId);
          if (found) {
  ...
  if (appPage === 'home') {
    return (
      <AnchorSelector
        onSelectAnchor={(anchor) => navigateTo('workspace', anchor)}
        onGoAnchorAdmin={() => navigateTo('anchor-admin')}
        onGoScriptAdmin={() => navigateTo('script-admin')}
        onGoModelAdmin={() => navigateTo('model-admin')}
        onGoTrainingAdmin={() => navigateTo('training-admin')}
      />
    );
  }
  ...
  if (appPage === 'anchor-admin') {
    return <AnchorAdminPage onBack={() => navigateTo('home')} />;
  }

  if (appPage === 'script-admin') {
    return <ScriptAdminPage onBack={() => navigateTo('home')} />;
  }

  if (appPage === 'model-admin') {
    return <ModelAdminPage onBack={() => navigateTo('home')} />;
  }

  if (appPage === 'training-admin') {
    return <TrainingAdminPage onBack={() => navigateTo('home')} />;
  }
  ...
  return (
    <div className={`min-h-screen font-sans print:bg-white print:h-auto overflow-x-hidden ${(step === AppStep.REPORT || step === AppStep.VERIFY_ANCHORS) ? '' : 'flex justify-center'}`} style={{ background: 'var(--bg, #F7F7F7)', color: 'var(--text, #111)' }}>
      <div className={`w-full min-h-screen flex flex-col relative print:max-w-none print:shadow-none print:min-h-0 transition-all duration-500 ease-in-out ${(step === AppStep.REPORT || step === AppStep.VERIFY_ANCHORS) ? 'max-w-none' : 'max-w-[600px]'}`} style={{ background: 'var(--surface, #fff)', boxShadow: '0 0 40px rgba(0,0,0,0.06)' }}>
...
// 登录视图 — Minimal AI + 微信绿风格
const LoginView: React.FC<{ onLogin: (u: string, p: string) => void; loading: boolean }> = ({ onLogin, loading }) => {
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');

  return (
    <div className="flex flex-col items-center justify-center py-20 px-6 md:px-8" style={{ minHeight: 'calc(100vh - 60px)' }}>
      <div className="w-full max-w-[300px] md:max-w-[360px]">
        {/* Logo + 品牌 */}
        <div className="flex items-center gap-3 mb-12">
          <div className="mint-logo w-10 h-10">
            <ShieldCheck size={20} className="text-white" />
          </div>
          <span className="text-lg font-black" style={{ color: 'var(--text)' }}>AI 智能质检</span>
        </div>

        {/* 标题 */}
        <h1 className="text-2xl font-black tracking-tight mb-1" style={{ color: 'var(--text)' }}>欢迎回来 👋</h1>
        <p className="text-sm mb-8" style={{ color: 'var(--text-3)' }}>登录管理控制台，开始智能质检工作</p>

        {/* 表单 */}
        <div className="space-y-5">
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5 ml-0.5" style={{ color: 'var(--text-3)' }}>管理员账号</label>
            <input
              type="text"
              value={username}
          ...
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5 ml-0.5" style={{ color: 'var(--text-3)' }}>访问密码</label>
            <input
              type="password"
              value={password}
          ...
          <button
            ...
            className="mint-btn-primary mt-6"
          >
            {loading ? (
          ...
          <p className="text-center text-[10px] font-medium tracking-widest uppercase" style={{ color: 'var(--text-4)' }}>
