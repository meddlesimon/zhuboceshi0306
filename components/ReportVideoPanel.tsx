...
const ReportVideoPanel: React.FC<Props> = ({ videoUrl, segments, onClose }) => {
    ...
    return (
      <div className="w-10 bg-slate-900 flex flex-col items-center py-3 gap-2 shrink-0">
        <button onClick={() => setIsCollapsed(false)}
          className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center text-white hover:bg-blue-500 transition-colors"
          title="展开视频面板">
  ...
  return (
    <div className="h-full bg-slate-900 flex flex-col border-r border-slate-800">
      {/* 顶部控制栏 */}
      <div className="flex items-center gap-2 px-3 py-1.5 border-b border-slate-700/50 shrink-0">
        <Video size={13} className="text-blue-400" />
        <span className="text-[11px] font-bold text-slate-300 flex-1">直播回放</span>
        <span className="text-[10px] text-slate-500 font-mono">{formatTime(currentTime)}/{formatTime(duration)}</span>
        <button onClick={() => setIsCollapsed(true)} className="w-5 h-5 rounded flex items-center justify-center text-slate-500 hover:text-slate-300" title="折叠">
          <ChevronLeft size={11} />
        </button>
        <button onClick={onClose} className="w-5 h-5 rounded flex items-center justify-center text-slate-500 hover:text-red-400" title="关闭">
          <X size={11} />
        </button>
      </div>

      {/* 视频播放器 — 占满剩余空间 */}
      <div className="flex-1 flex items-start justify-center p-1 overflow-hidden">
        <video
          ref={videoRef}
          src={videoUrl}
      ...
      {/* 底部提示 */}
      <div className="px-2 py-1.5 text-center shrink-0 border-t border-slate-800">
        <p className="text-[9px] text-slate-600">右侧选中文字 → 「定位视频」跳转</p>
      </div>
    </div>
  );
};
