...
type WorkspaceTab = 'replay' | 'qc' | 'zhongkong';


// ===== 中控质检场次列表 =====
const ZhongkongSessionList: React.FC<{anchor: any}> = ({ anchor }) => {
  ...
  const [analyzing, setAnalyzing] = React.useState<number | null>(null);
  ...
  const loadSessions = async () => {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        const statusRes = await fetch('/api/zhongkong/batch-status', {
          method: 'POST',
  ...
  const runAnalysis = async (sessionId: number) => {
    ...
    try {
      await fetch('/api/zhongkong/analyze/' + sessionId, { method: 'POST' });
      setStatusMap(prev => ({ ...prev, [sessionId]: 'done' }));
    } catch {}
    setAnalyzing(null);
  };

  const viewReport = async (session: any) => {
    try {
      const res = await fetch('/api/zhongkong/report/' + session.id);
      const data = await res.json();
      setViewingReport({ ...data, session });
    } catch {}
  };

  if (viewingReport) {
    return <ZhongkongReportPage
      sessionId={viewingReport.session.id}
      sessionTitle={viewingReport.session.title || ''}
      sessionDate={viewingReport.session.live_date || viewingReport.session.created_at || ''}
    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-6">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-xs text-slate-400">{sessions.length} 场直播</span>
        </div>
        {loading ? (
          ...
          <div className="text-center py-20 text-slate-400 text-sm">暂无直播记录</div>
        ) : (
          <div className="space-y-2">
            {sessions.map((s: any) => {
              ...
              return (
                <div key={s.id} className="bg-white rounded-xl border border-slate-100 px-4 py-3 flex items-center justify-between hover:shadow-sm transition-shadow">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-slate-800">{fmtTime(s.created_at)}</span>
                      {s.duration_seconds > 0 && (
                        <span className="text-xs text-slate-400">{fmtDuration(s.duration_seconds)}</span>
                      )}
                    </div>
                    <p className="text-xs text-slate-400 truncate mt-0.5">#{s.id} · {s.title || '直播场次'}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {status === 'done' ? (
                      <>
                        <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full">已质检</span>
                        <button onClick={() => viewReport(s)}
                          className="text-xs font-bold text-[#07C160] hover:bg-[#07C160]/10 px-3 py-1.5 rounded-lg transition-colors">
                          查看报告
                        </button>
                      </>
                    ) : (
                      <button
                        ...
                        className="text-xs font-bold text-white bg-[#07C160] hover:bg-[#06AD56] px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 flex items-center gap-1"
                      >
                        {isAnalyzing ? <><Loader2 size={12} className="animate-spin" /> 分析中...</> : '执行质检'}
                      </button>
                    )}
                  </div>
                </div>
    ...
    <div className="min-h-screen bg-[#F7F7F7] flex flex-col">
      ...
      <div className="bg-white border-b border-slate-200 px-3 md:px-6 flex items-center justify-center shrink-0 shadow-sm">
        <div className="flex gap-1 py-1">
          ...
          {enableQc && (
            <button
              onClick={() => setActiveTab('zhongkong')}
              className={`flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-lg transition-all ${
                activeTab === 'zhongkong'
                  ? 'bg-[#07C160] text-white shadow-md'
                  : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
