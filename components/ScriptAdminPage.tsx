...
const ScriptAdminPage: React.FC<Props> = ({ onBack }) => {
  ...
  const typeLabel = (s: Standard) => {
    ...
    if (s.importance === 'high') return <span className="px-2 py-0.5 bg-[rgba(7,193,96,0.1)] text-[#07C160] text-[10px] font-bold rounded-full">今日重点</span>;
    return <span className="px-2 py-0.5 bg-[#F7F7F7] text-slate-500 text-[10px] font-bold rounded-full">日常</span>;
  };
    ...
    <div className="min-h-screen bg-[#F7F7F7] flex flex-col">
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-4 flex items-center gap-4 shadow-sm">
        ...
        <div className="ml-auto">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#07C160] hover:bg-[#06AD56] text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-[rgba(7,193,96,0.1)]"
          >
            <Upload size={14} />
            上传新版本话术
          </button>
      ...
      <div className="flex flex-1 overflow-hidden" style={{ height: 'calc(100vh - 73px)' }}>
        ...
        <div className="w-64 bg-white border-r border-slate-100 flex flex-col shrink-0">
          ...
          {loading ? (
            <div className="flex justify-center pt-8"><Loader2 size={24} className="text-[#07C160] animate-spin" /></div>
          ) : versions.length === 0 ? (
            ...
            <div className="flex-1 overflow-y-auto">
              {versions.map(ver => (
                <div
                  key={ver.id}
                  className={`w-full text-left px-4 py-4 border-b border-slate-50 hover:bg-[#F7F7F7] transition-colors cursor-pointer group ${selectedVersion?.id === ver.id ? 'bg-[rgba(7,193,96,0.06)] border-l-2 border-l-[#07C160]' : ''}`}
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      {ver.is_current === 1 ? (
                        <span className="text-[10px] font-bold bg-[#07C160] text-white px-2 py-0.5 rounded-full">当前</span>
                      ) : (
                        <button
              ...
              <div className="bg-white border-b border-slate-100 px-5 py-3 flex items-center justify-between gap-3 shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  {selectedVersion.is_current === 1 && (
                    <span className="text-[10px] font-bold bg-[#07C160] text-white px-2 py-0.5 rounded-full">当前使用中</span>
                  )}
                </div>
                <div className="flex gap-1">
                  {(['all', 'high', 'normal', 'forbidden'] as const).map(f => (
                    <button
                      key={f}
                      onClick={() => setFilter(f)}
                      className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${filter === f ? 'bg-slate-900 text-white' : 'bg-[#F7F7F7] text-slate-500 hover:bg-slate-200'}`}
                    >
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredContent.map((s, idx) => (
                      <tr key={s.id || idx} className="hover:bg-[#F7F7F7]/50 transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-[500px] bg-white flex flex-col shadow-2xl animate-in slide-in-from-right-8 duration-300">
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowUploadPanel(false)} className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-[#F7F7F7] transition-colors text-slate-400 text-xl font-light">×</button>
            </div>

            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              {/* 第一步：飞书链接 */}
              <div className="bg-[rgba(7,193,96,0.06)] border border-[rgba(7,193,96,0.15)] rounded-2xl p-4">
                <div className="flex items-center gap-2 text-[#06AD56] font-bold text-sm mb-3">
                  <span className="w-5 h-5 bg-[#07C160] text-white rounded-full text-xs flex items-center justify-center">1</span>
                  打开飞书表格，全选复制
                </div>
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-[#07C160] hover:bg-[#06AD56] text-white font-bold text-sm py-3 rounded-xl transition-all"
                >
              ...
              <div>
                ...
                <div className="relative">
                  <textarea
                    ...
                    className="w-full h-36 p-4 border-2 border-slate-200 rounded-xl text-xs font-mono leading-relaxed outline-none focus:border-[#07C160] focus:bg-white transition-all resize-none"
                  />
                  <div className="absolute top-3 right-3 bg-white/85 px-2 py-1 rounded text-[10px] font-bold text-slate-400 border border-slate-200 flex items-center gap-1">
              ...
              <button
                ...
                className="w-full py-3 border-2 border-dashed border-slate-300 hover:border-[#07C160] text-slate-600 hover:text-[#07C160] font-bold text-sm rounded-xl transition-all disabled:opacity-50"
              >
                解析预览
              </button>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex gap-3">
              <button onClick={() => setShowUploadPanel(false)} className="flex-1 py-3 border border-slate-200 rounded-xl text-sm font-bold text-slate-500 hover:bg-[#F7F7F7] transition-all">
                取消
              </button>
              <button
                onClick={handleUpload}
                disabled={!parsedStandards || uploading}
                className="flex-1 py-3 bg-[#07C160] hover:bg-[#06AD56] text-white rounded-xl text-sm font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-md shadow-[rgba(7,193,96,0.1)]"
              >
