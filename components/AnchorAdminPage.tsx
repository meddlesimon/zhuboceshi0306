...
const AnchorAdminPage: React.FC<Props> = ({ onBack }) => {
  ...
  const [editForm, setEditForm] = useState({ name: '', enable_qc: true, douyin_profile_url: '', douyin_room_url: '' });
  ...
  const startEdit = (anchor: Anchor) => {
    ...
    setEditForm({
      name: anchor.name,
      enable_qc: anchor.enable_qc !== false,
            ...
            <div className="divide-y divide-slate-50">
              {anchors.map((anchor, idx) => (
                <div key={anchor.id}>
                  ...
                  <div className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50/50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {/* 类型标签 */}
                        {anchor.enable_qc !== false ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-full bg-blue-50 text-blue-600 border border-blue-100">
                  ...
                  {/* 编辑面板（展开时显示） */}
                  {editingId === anchor.id && (
                    <div className="px-5 pb-5 bg-slate-50/50 border-t border-slate-100 animate-in slide-in-from-top-2 duration-200">
                      <div className="max-w-lg space-y-4 pt-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-blue-400 transition-all"
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-between bg-white rounded-xl border-2 border-slate-200 px-4 py-3">
                          ...
                          <button
                            ...
                            className={`relative w-12 h-7 rounded-full transition-colors ${editForm.enable_qc ? 'bg-blue-600' : 'bg-slate-300'}`}
                          >
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-blue-400 transition-all font-mono"
                          />
                        </div>
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                          ...
                          <button
                            onClick={handleSave}
                            disabled={saving}
                            className="flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl transition-all disabled:opacity-50 shadow-md shadow-blue-100"
                          >
