    ...
    <div className="min-h-screen bg-[#F7F7F7] flex flex-col">
      ...
      <div className="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="p-2 rounded-xl hover:bg-[#F7F7F7] transition-colors text-slate-500">
            <ArrowLeft size={18} />
          </button>
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 bg-gradient-to-br from-[#07C160] to-[#059352] rounded-xl flex items-center justify-center shadow">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-white bg-[#07C160] hover:bg-[#06AD56] rounded-xl transition-all"
        >
          <Plus size={14} /> 添加管理员
        </button>
      </div>
      ...
      <div className="flex-1 p-6 max-w-2xl mx-auto w-full">
        ...
        {/* 添加表单 */}
        {showAddForm && (
          <div className="bg-white rounded-2xl border-2 border-[rgba(7,193,96,0.2)] p-5 mb-4 shadow-sm">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={newDisplayName} onChange={e => setNewDisplayName(e.target.value)} placeholder="例如：张浩辉"
                  className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl outline-none focus:border-[#07C160]" />
              </div>
              <div>
                ...
                <input value={newUsername} onChange={e => setNewUsername(e.target.value)} placeholder="例如：zhanghaohui"
                  className="w-full px-3 py-2 text-sm border border-slate-200 rounded-xl outline-none focus:border-[#07C160] font-mono" />
              </div>
              <div>
              ...
              <button onClick={handleAdd} disabled={adding}
                className="w-full py-2.5 bg-[#07C160] hover:bg-[#06AD56] text-white text-sm font-bold rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {accounts.map(acc => (
              <div key={acc.id} className={`bg-white rounded-2xl border-2 p-4 transition-all ${acc.role === 'super_admin' ? 'border-[rgba(7,193,96,0.2)] shadow-sm' : 'border-slate-100'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${acc.role === 'super_admin' ? 'bg-[rgba(7,193,96,0.1)]' : 'bg-[#F7F7F7]'}`}>
                    {acc.role === 'super_admin' ? <Shield size={18} className="text-[#07C160]" /> : <User size={18} className="text-slate-400" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-black text-slate-800">{acc.display_name}</h3>
                      {acc.role === 'super_admin' && (
                        <span className="px-2 py-0.5 bg-[rgba(7,193,96,0.1)] text-[#06AD56] text-[10px] font-bold rounded-full">超级管理员</span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <button onClick={() => { setResetId(resetId === acc.id ? null : acc.id); setResetPwd(''); }}
                      className="p-1.5 text-slate-400 hover:text-[#07C160] hover:bg-[rgba(7,193,96,0.06)] rounded-lg transition-all" title="重置密码">
                      <Key size={14} />
                    </button>
                    {/* 删除（超级管理员不可删） */}
                    {acc.role !== 'super_admin' && (
                ...
                {resetId === acc.id && (
                  <div className="mt-3 pt-3 border-t border-slate-100 flex items-center gap-2">
                    <input value={resetPwd} onChange={e => setResetPwd(e.target.value)} placeholder="输入新密码"
                      className="flex-1 px-3 py-1.5 text-sm border border-slate-200 rounded-lg outline-none focus:border-[#07C160] font-mono" />
                    <button onClick={() => handleResetPwd(acc.id)}
                      className="px-3 py-1.5 text-xs font-bold text-white bg-[#07C160] hover:bg-[#07C160] rounded-lg transition-all">确认重置</button>
