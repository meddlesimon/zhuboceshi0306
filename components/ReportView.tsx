...
interface ReportViewProps {
  ...
  taskId?: string; // 任务 ID，用于人工手检 API
}
                    ...
                    {options.map((val) => (
                      <button 
                        ...
                        className={`px-2 py-1 rounded-md text-[10px] font-black transition-all ${
                          currentScore === val 
                          ? 'bg-[#07C160] text-white shadow-sm scale-105' 
                          : 'text-slate-400 hover:bg-white hover:text-[#07C160]'
                        }`}
                      >
                        {val}
                      </button>
...
const ReportView: React.FC<ReportViewProps> = ({ result, standards, metadata, onReset, onRegisterActions, videoSegments, taskId }) => {
  ...
  const [showAdminAudit, setShowAdminAudit] = useState(true); // 控制右侧面板显示（默认展开）
  const [rightPanelTab, setRightPanelTab] = useState<'audit' | 'transcript'>('transcript'); // 右侧面板模式切换
  ...
  const manualChecksRef = useRef<HTMLDivElement>(null);

  // 加载人工手检
  useEffect(() => {
    if (!taskId) return;
    fetch(`/api/tasks/${taskId}/manual-checks`)
      .then(r => r.json())
      .then(data => { if (Array.isArray(data)) setManualChecks(data); })
      .catch(() => {});
  }, [taskId]);

  // 添加人工手检
  const addManualCheck = async (selectedText: string) => {
    if (!taskId) return;
    try {
      const r = await fetch(`/api/tasks/${taskId}/manual-checks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ selected_text: selectedText, time_range: '', comment: '' })
      });
      const item = await r.json();
      if (item.id) {
        setManualChecks(prev => [...prev, item]);
        // 滚动到报告底部人工手检区域
        setTimeout(() => manualChecksRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 200);
      }
    } catch (e) { console.error('添加人工手检失败', e); }
  };

  // 更新人工手检备注（防抖保存）
  const updateManualCheckComment = async (mcId: string, comment: string) => {
    if (!taskId) return;
    setManualChecks(prev => prev.map(m => m.id === mcId ? { ...m, comment } : m));
    try {
      await fetch(`/api/tasks/${taskId}/manual-checks/${mcId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ comment })
      });
    } catch (e) { console.error('更新人工手检失败', e); }
  };

  // 删除人工手检
  const deleteManualCheck = async (mcId: string) => {
    if (!taskId) return;
    setManualChecks(prev => prev.filter(m => m.id !== mcId));
    try {
      await fetch(`/api/tasks/${taskId}/manual-checks/${mcId}`, { method: 'DELETE' });
    } catch (e) { console.error('删除人工手检失败', e); }
  };
  // 中间报告 vs 右侧面板的可拖拽宽度
    ...
    onRegisterActions(
      <>
          ...
          <>
            <div className="bg-slate-100 p-0.5 rounded-lg flex">
              <button
                ...
                className={`px-3 py-1 text-[11px] font-bold rounded-md transition-all ${activeRound === 'round1' ? 'bg-white shadow-sm text-[#07C160]' : 'text-slate-500 hover:text-slate-700'}`}
              >
                第一轮
              </button>
              <button
        ...
        <button
          ...
          className={`bg-[#07C160] hover:bg-[#06AD56] text-white px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 shadow-md transition-all active:scale-95 text-xs ${isExporting ? 'opacity-75 cursor-wait' : ''}`}
        >
        ...
        <div className="space-y-0">
          {segWithOffsets.map((seg, idx) => (
            <div key={idx} className="border-b border-slate-100 last:border-b-0">
              {/* 时间标签 — 在上方，可点击跳转 */}
              <div
                className="bg-slate-50 px-3 py-1 cursor-pointer hover:bg-[rgba(7,193,96,0.06)] transition-colors flex items-center gap-1.5"
                onClick={() => {
              ...
              >
                <span className="text-[10px] text-[#07C160] font-mono font-bold">
  ...
  return (
    <div className="animate-in fade-in duration-500 bg-slate-50 h-full">
        ...
        >
          ...
          <div className="space-y-12 animate-in fade-in duration-300" key={activeRound}>
            ...
            {manualChecks.length > 0 && (
              <div ref={manualChecksRef} className="mt-8 space-y-4" id="manual-checks-section">
                <div className="flex items-center gap-3 mb-4">
                  <div className="bg-orange-100 p-2 rounded-lg text-orange-600">
                    <AlertTriangle size={20} />
                  </div>
                  <div>
                    <h3 className="text-lg font-black text-slate-900">人工手检</h3>
                    <p className="text-sm text-slate-500 font-medium">运营人员手动标记的问题 ({manualChecks.length} 条)</p>
                  </div>
                </div>

                <div className="space-y-3">
                  {manualChecks.map((mc, idx) => (
                    <div
                      key={mc.id}
                      className="bg-white rounded-2xl border-l-4 border-l-orange-400 border border-slate-100 shadow-sm overflow-hidden"
                    >
                      {/* 卡片头部 */}
                      <div className="bg-orange-50 px-4 py-2 flex items-center justify-between border-b border-orange-100">
                        <div className="flex items-center gap-2">
                          <span className="w-1.5 h-1.5 bg-orange-500 rounded-full"></span>
                          <span className="text-[11px] font-black text-orange-700 uppercase tracking-wider">人工手检 #{idx + 1}</span>
                          <span className="text-[10px] text-slate-400">{mc.created_at}</span>
                        </div>
                        <button
                          onClick={() => { if (confirm('确定删除此手检条目？')) deleteManualCheck(mc.id); }}
                      ...
                      <div className="p-4 space-y-3">
                        {/* 框选的原文 */}
                        <div>
                          <p className="text-[9px] font-bold text-orange-400 uppercase tracking-wider mb-1">标记原文</p>
                          <div
                            onClick={() => handleQuoteClick(mc.selected_text, `mc-${mc.id}`)}
                            className="bg-orange-50 border border-orange-100 rounded-xl p-3 text-sm text-slate-700 italic leading-relaxed cursor-pointer hover:border-orange-300 transition-colors"
                          >
                            "{mc.selected_text}"
                            <div className="mt-2 flex items-center gap-1 text-[10px] font-bold text-orange-400 not-italic">
                        ...
                        {/* 运营备注输入框 */}
                        <div>
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">问题说明</p>
                          <textarea
                            value={mc.comment}
                            onChange={(e) => updateManualCheckComment(mc.id, e.target.value)}
                            placeholder="请输入此处存在的问题说明...（将随报告导出）"
                            className="w-full text-sm p-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-300 h-20 resize-none placeholder:text-slate-300 transition-all"
                          />
                        </div>
                      </div>
                    </div>
      ...
      {showAdminAudit && (
        <>
          ...
          <div
            ...
            className="w-[5px] shrink-0 cursor-col-resize bg-slate-200 hover:bg-[#07C160] active:bg-[#06AD56] transition-colors relative group z-10"
            title="拖拽调整宽度"
          >
          ...
          <div className={`bg-white rounded-2xl border-2 shadow-2xl overflow-hidden flex flex-col h-full flex-1 ${rightPanelTab === 'audit' ? 'border-orange-200' : 'border-[rgba(7,193,96,0.2)]'}`}>
            
            {/* 动态 Header */}
            <div className={`${rightPanelTab === 'audit' ? 'bg-orange-500' : 'bg-[#07C160]'} p-4 text-white flex justify-between items-center shrink-0`}>
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-3">
                 {/*<div className="bg-black/20 rounded-lg p-0.5 flex">
                    ...
                    <button 
                      ...
                      className={`px-3 py-1 text-[10px] font-bold rounded transition-all ${rightPanelTab === 'transcript' ? 'bg-white text-[#07C160] shadow-sm' : 'text-white/70 hover:text-white'}`}
                    >全文</button>
                 </div>*/}
                ...
                <div className="flex-1 flex flex-col h-full relative">
                    ...
                    {result.isDualMode && (
                      <div className="p-2 flex gap-1 bg-slate-50/50">
                        <button 
                          ...
                          className={`flex-1 py-1.5 text-[10px] font-black rounded border transition-all ${activeRound === 'round1' ? 'bg-[#07C160] border-[#07C160] text-white' : 'bg-white text-slate-400 border-slate-200 hover:border-[rgba(7,193,96,0.2)]'}`}
                        >第一轮原文</button>
                        <button 
                          onClick={() => setActiveRound('round2')}
                          className={`flex-1 py-1.5 text-[10px] font-black rounded border transition-all ${activeRound === 'round2' ? 'bg-[#07C160] border-[#07C160] text-white' : 'bg-white text-slate-400 border-slate-200 hover:border-[rgba(7,193,96,0.2)]'}`}
                        >第二轮原文</button>
                      </div>
                    )}
                    ...
                    <div className="p-3 flex items-center gap-2 bg-white">
                      <div className="relative flex-1">
                        ...
                        <input 
                          ...
                          placeholder="搜索全文关键字..."
                          className="w-full pl-9 pr-4 py-2 bg-slate-100 border-none rounded-xl text-sm focus:ring-2 focus:ring-[#07C160] transition-all font-medium"
                          onKeyDown={(e) => {
                  ...
                  <div className="flex-1 relative flex overflow-hidden h-full">
                    <div 
                      ...
                      className="flex-1 overflow-y-auto p-8 font-mono text-sm leading-[1.8] text-slate-600 whitespace-pre-wrap selection:bg-green-200 selection:text-green-900 scroll-smooth bg-white h-full"
                      onMouseUp={(e) => {
                    ...
                    {selectionPopup && (
                      ...
                      >
                        <div className="flex items-center gap-1 bg-white rounded-xl shadow-2xl border border-slate-200 p-1">
                          <button
                            onMouseDown={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 bg-[#07C160] hover:bg-[#06AD56] text-white text-xs font-bold rounded-lg transition-colors whitespace-nowrap"
                          >
                            <MapPin size={12} /> 定位视频
                          </button>
                          {taskId && (
                            <button
                              onMouseDown={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                addManualCheck(selectionPopup.text);
                                setSelectionPopup(null);
                                window.getSelection()?.removeAllRanges();
                              }}
                              className="flex items-center gap-1.5 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold rounded-lg transition-colors whitespace-nowrap"
                            >
                              <AlertTriangle size={12} /> 人工手检
                            </button>
                          )}
                        </div>
                        <div className="w-3 h-3 bg-white rotate-45 mx-auto -mt-1.5 border-r border-b border-slate-200" />
                      </div>
                    )}
                    ...
                    <div className="w-2.5 bg-slate-50 border-l border-slate-100 relative shrink-0 z-10">
                      ...
                      {searchMatches.map((pos, i) => {
                        ...
                        return (
                          <div 
                            key={i}
                            className={`absolute left-0 w-full h-0.5 ${i === currentMatchIndex ? 'bg-orange-500 z-10 h-1' : 'bg-[#07C160] opacity-60'}`}
                            style={{ top: `${top}%` }}
                          />
                        );
                      })}
      ...
      <button
        ...
        className={`fixed bottom-10 right-10 z-50 flex items-center gap-2 bg-slate-800 hover:bg-[#07C160] text-white px-5 py-3 rounded-full shadow-2xl transition-all duration-300 transform font-bold print:hidden ${
