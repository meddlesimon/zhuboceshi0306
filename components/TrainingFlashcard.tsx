...
import {
  ...
  Eye, EyeOff, Expand, GraduationCap, Loader2, LogOut, ShieldCheck, X, CornerDownRight, Monitor
} from 'lucide-react';
...
const TrainingFlashcard: React.FC = () => {
  ...
  // 主 Tab：直播观摩 / 闪卡学习
  const [mainTab, setMainTab] = useState<'live' | 'courses'>('live');
...
const CourseSelectPage: React.FC<{
  displayName: string;
  mainTab: 'live' | 'courses';
  onTabChange: (tab: 'live' | 'courses') => void;
  onSelect: (c: TrainingCourse) => void;
  onLogout: () => void;
}> = ({ displayName, mainTab, onTabChange, onSelect, onLogout }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-teal-50/40 flex flex-col">
      {/* 顶部栏 */}
      <div className="bg-white border-b border-teal-100 px-4 md:px-6 py-3 flex items-center justify-between shadow-sm shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm md:text-base font-black text-slate-900">学习机知识培训系统</h1>
      ...
      {/* Tab 切换：直播观摩 | 闪卡学习 */}
      <div className="bg-white border-b border-teal-100 flex shrink-0">
        <button
          onClick={() => onTabChange('live')}
          className={`flex-1 py-3 text-sm font-bold text-center transition-all flex items-center justify-center gap-1.5 ${
            mainTab === 'live'
      ...
      {/* Tab 内容 */}
      {mainTab === 'live' ? (
        <TrainingLiveView onBack={() => onTabChange('courses')} />
      ) : (
