...
app.get('/api/qa/:sessionId', (req, res) => {
  ...
  try {
    if (!existsSync(qaFile)) return res.json([]);
    const data = JSON.parse(readFileSync(qaFile, 'utf8'));
    res.json(data);
  } catch (e) {
...
app.post('/api/qa/:sessionId', (req, res) => {
  ...
  let records = [];
  try {
    if (existsSync(qaFile)) records = JSON.parse(readFileSync(qaFile, 'utf8'));
  } catch {}
  const newRecord = {
    id: 'qa_' + Date.now(),
    question,
  ...
  writeFileSync(qaFile, JSON.stringify(records, null, 2));
  res.json(newRecord);
});
...
// 获取中控质检配置
app.get('/api/zhongkong/config', (req, res) => {
  const configFile = path.join(DATA_DIR, 'zhongkong_config.json');
  try {
    ...
    res.json(JSON.parse(readFileSync(configFile, 'utf8')));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
// 保存中控质检配置
app.post('/api/zhongkong/config', (req, res) => {
  const configFile = path.join(DATA_DIR, 'zhongkong_config.json');
  try {
    writeFileSync(configFile, JSON.stringify(req.body, null, 2));
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// 对指定 session 执行中控质检分析
app.post('/api/zhongkong/analyze/:sessionId', (req, res) => {
  const sid = req.params.sessionId;
  const commentsFile = path.join(DATA_DIR, 'danmaku', 'session_' + sid + '_comments.json');
  const reportDir = path.join(DATA_DIR, 'zhongkong');
  const reportFile = path.join(reportDir, 'session_' + sid + '.json');
  ...
  try {
    ...
    let config = {
      officialPrefix: '北大孙叶夫妇',
      agentIds: [],
      targets: { head: { instruction: 350, rendering: 20 }, normal: { instruction: 245, rendering: 14 } }
    };
    if (existsSync(configFile)) {
    ...
    if (!existsSync(commentsFile)) {
      return res.json({ hasData: false, message: '该场次暂无弹幕数据' });
    }
    const comments = JSON.parse(readFileSync(commentsFile, 'utf8'));
    if (!Array.isArray(comments) || comments.length === 0) {
      return res.json({ hasData: false, message: '弹幕数据为空' });
    }
    
    // 识别水军账号：通过配置的 agentIds 列表、昵称首字前缀、或昵称含 *
    const agentIdSet = new Set((config.agentIds || []).map(a => a.shortId || a.id || ''));
    const officialPrefix = config.officialPrefix || '北大孙叶夫妇';
    // 水军昵称首字前缀（与 zhongkong 原版一致）
    const monitorPrefixes = new Set(config.monitorPrefixes || ['北','一','好','6','鲸','阿','笑','爱','我','重','-','D','*','马']);
    
    // 分类每条弹幕
    const classified = [];
    for (const c of comments) {
      const nn = (c.n || '').trim();
      const content = (c.c || '').trim();
      if (!nn || !content) continue;
      
      // 判定官方回复
      if (nn.startsWith(officialPrefix)) {
        classified.push({ ...c, cat: 'official', nn });
        continue;
      }
      
      // 判定是否是水军账号：首字在白名单中、或在 agentIds 中、或含 *
      const firstChar = nn.charAt(0);
      const isAgent = monitorPrefixes.has(firstChar) || nn.includes('*') || agentIdSet.has(nn);
      if (!isAgent) continue; // 只分析水军账号
      
      // 判定指令回应：长度≤3 或 纯数字
      const trimContent = content.trim();
      const isInstruction = (trimContent.length <= 3 && trimContent.length > 0) || /^\d+$/.test(trimContent);
      
      classified.push({
        ...c,
        cat: isInstruction ? 'instruction' : 'rendering',
        nn
      });
    }
    ...
    for (const item of classified) {
      ...
      const acc = accountMap[item.nn];
      acc.total++;
      acc[item.cat]++;
      acc.comments.push({ t: item.t, c: item.c, cat: item.cat });
    }
    
    const accountStats = Object.values(accountMap).sort((a, b) => b.total - a.total);
    
    // 总计
    const totals = {
      instruction: classified.filter(c => c.cat === 'instruction').length,
      rendering: classified.filter(c => c.cat === 'rendering').length,
      official: classified.filter(c => c.cat === 'official').length,
    ...
    const report = {
      ...
      totals,
      accountStats,
      config: { officialPrefix, targets: config.targets },
    ...
    console.error('[中控质检] 分析错误:', e.message);
    res.status(500).json({ error: e.message });
  }
});

// 获取已有的中控质检报告
app.get('/api/zhongkong/report/:sessionId', (req, res) => {
  const sid = req.params.sessionId;
  const reportFile = path.join(DATA_DIR, 'zhongkong', 'session_' + sid + '.json');
  try {
    if (!existsSync(reportFile)) {
      return res.json({ hasData: false, message: '尚未执行质检' });
    }
    res.json(JSON.parse(readFileSync(reportFile, 'utf8')));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
// 批量获取中控质检状态
app.post('/api/zhongkong/batch-status', (req, res) => {
  const { session_ids } = req.body;
  if (!Array.isArray(session_ids)) return res.json({});
  const result = {};
  for (const sid of session_ids) {
    const reportFile = path.join(DATA_DIR, 'zhongkong', 'session_' + sid + '.json');
    result[sid] = existsSync(reportFile) ? 'done' : 'none';
  }
  res.json(result);
});
